...
reutilizável `LotteryMetricEngine<Profile>`. A entrada aceita exclusivamente
...
Os limiares absolutos do PRD são aplicados somente a cartelas simples de 15
dezenas. Em apostas de 16 a 20, as métricas continuam disponíveis, porém cada
flag declara `applicable: false` e `isExtreme: null`. Isso evita reutilizar
um limite de 15 dezenas como se fosse uma distribuição teórica de outro
tamanho. A definição de extremos normalizados para essas apostas pertence a
uma story posterior e deve ser versionada.
